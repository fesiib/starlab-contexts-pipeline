...
class VideoPool:
    ...
    def __cluster_v2(self, items, similarity_threshold, summarization_f):
        if len(items) == 0:
            return []
        texts = []
        for item in items:
            texts.append(item["title"] + ": " + item[f"description"])
        # tags = []
        # for item in items:
        ...
        for video_id, all_alignments in alignments_per_video.items():
            ...
            for subgoal_aspect, alignments in alignments_per_subgoal_aspect.items():
                ...
                def __get_notable(links):
                    ...
                    return summary
                new_notables = self.__cluster_v2(alignments, SIMILARITY_THRESHOLD_NOTABLE, __get_notable)
        ...
        for key, links in links_to.items():
            ...
            if approach == "llm":
                ...
                def __get_hook(links):
                    ...
                    return summary
                new_hooks = self.__cluster_v2(links, SIMILARITY_THRESHOLD_HOOK, __get_hook)
