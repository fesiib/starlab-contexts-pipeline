...
def update_facet_candidates(task, cell_to_units, include_cells, pieces_at_once, embedding_method, generation_model):
    """
    ...
    for chosen_pieces in chosen_sets_of_pieces:
        ...
        retrieved_candidates = form_segmentation_facet_candidates(task, chosen_pieces, generation_model)
    ...
    if len(chosen_sets_of_pieces) > 1:
        ...
        new_facet_candidates = combine_segmentation_facet_candidates(task, new_facet_candidates, generation_model)
...
def update_facet_labels(task, labeled_dataset, facet_candidates, vocabulary_iterations, generation_model):
    ...
    for facet in facet_candidates:
        ...
        for video in [labeled_dataset[i] for i in selected_indices]:
            vocabulary = form_codebook(task, video["transcript"], facet, generation_model)
            facet["vocabulary"] = vocabulary
        facet["vocabulary"] = try_adding_na_label(task, facet["vocabulary"], generation_model)
...
def update_labeled_dataset(task, labeled_dataset, facet_candidates, generation_model):
    ...
    for facet in facet_candidates:
        ...
        for video in labeled_dataset:
            ...
            labeled_pieces = label_transcript_pieces(task, video["pieces"], facet, generation_model)
