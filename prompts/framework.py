...
def form_codebook(task, transcript, facet, generation_model):
    transcript_str = transcript_to_str(transcript)
    ...
    response = get_response_pydantic(messages, VocabularySchema, model=generation_model)
...
def try_adding_na_label(task, vocabulary, generation_model):
    iterations = 0
    while iterations < 10:
...
def label_transcript_pieces(task, pieces, facet, generation_model):
    ...
    response = get_response_pydantic(messages, LabeledPiecesSchema, model=generation_model)
...
def form_segmentation_facet_candidates(task, pieces, generation_model):
    ...
    response = get_response_pydantic(messages, CandidateSegmentationFacetsSchema, model=generation_model)
...
def combine_segmentation_facet_candidates(task, all_candidates, generation_model):
